...
public class BarcodeCaptureFragment extends Fragment implements View.OnTouchListener {
    ...
    public static BarcodeCaptureFragment instantiate(WfhBarcodeScanner.ScanningMode mode,
    ...
    public static BarcodeCaptureFragment instantiate(WfhBarcodeScanner.ScanningMode mode, CameraSourcePreview.PreviewScaleType scaleType,
                                                     @WfhBarcodeScanner.BarCodeFormat int... formats) {
        BarcodeCaptureFragment fragment = instantiate(mode, formats);
        fragment.getArguments().putSerializable(WfhBarcodeScanner.PREVIEW_SCALE_TYPE, scaleType);
        return fragment;
    }
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (getArguments() != null && !getArguments().isEmpty()) {
            ...
            if (getArguments().containsKey(WfhBarcodeScanner.PREVIEW_SCALE_TYPE))
                mPreviewScaleType = (CameraSourcePreview.PreviewScaleType) getArguments().getSerializable(WfhBarcodeScanner.PREVIEW_SCALE_TYPE);
        }
    }
    ...
    protected boolean onTap(float rawX, float rawY) {
        ...
        if (mMode == WfhBarcodeScanner.ScanningMode.SINGLE_AUTO) {
        ...
        } else if (mMode == WfhBarcodeScanner.ScanningMode.SINGLE_MANUAL) {
    ...
    protected void onBarcodeDetected(final Barcode barcode){
        ...
        if (mMode == WfhBarcodeScanner.ScanningMode.SINGLE_AUTO && mListener != null) {
            synchronized (mLock){
                if(!isListenerBusy) {
                    isListenerBusy = true;
